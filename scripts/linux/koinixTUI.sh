        ...
        case $choice in
            ...
            2)
                ...
                if [[ "$pkg_manager" == "apt-get" ]]; then
                    ...
                    sudo yum install chromium -y || sudo dnf install chromium -y
            ...
            3)
                ...
                if [[ "$pkg_manager" == "apt-get" ]]; then
                    ...
                    sudo yum install chromium-bsu -y || sudo dnf install chromium-bsu -y
                elif [[ "$pkg_manager" == "pacman" ]]; then
                    sudo pacman -S chromium-bsu --noconfirm
                elif [[ "$pkg_manager" == "zypper" ]]; then
                    sudo zypper install chromium-bsu
                fi
                confirm_return
                ;;
            4)
...
function install_dependencies {
    ...
    while true; do
        choice=$(dialog --clear --backtitle "Koishi Manager" \
                        --title "安装依赖" \
                        --menu "请选择要安装的依赖 (需要 sudo 权限):" 18 60 10 \
                        1 "安装 Node.js" \
                        2 "安装 Yarn" \
        ...
        case $choice in
            1)
                ...
                if [[ "$pkg_manager" == "apt-get" ]]; then
                    sudo apt-get update
                    sudo apt-get install nodejs npm -y # 包含 npm
            ...
            2)
                ...
                if [[ "$pkg_manager" == "apt-get" ]]; then
                    ...
                    sudo yum install yarn -y || sudo dnf install yarn -y # 尝试 yum 和 dnf
                elif [[ "$pkg_manager" == "pacman" ]]; then
            ...
            4)
                ...
                if [[ "$pkg_manager" == "apt-get" ]]; then
                    ...
                    sudo yum install ffmpeg -y || sudo dnf install ffmpeg -y
                elif [[ "$pkg_manager" == "pacman" ]]; then
