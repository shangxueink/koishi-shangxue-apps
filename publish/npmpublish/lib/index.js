...
var Config = import_koishi.Schema.intersect([
  import_koishi.Schema.object({
    ...
    maximum_times_per_day: import_koishi.Schema.number().description("每日签到次数上限`小鹿怡..什么伤身来着`").default(3).min(1),
  ...
  import_koishi.Schema.object({
    ...
    cost: import_koishi.Schema.object({
      checkin_reward: import_koishi.Schema.array(import_koishi.Schema.object({
        command: import_koishi.Schema.union(["鹿", "鹿@用户", "补鹿", "戒鹿", "补鹿@用户", "戴锁"]).description("交互指令"),
    ...
    special_cost: import_koishi.Schema.dict(import_koishi.Schema.object({
      checkin_reward: import_koishi.Schema.array(import_koishi.Schema.object({
        command: import_koishi.Schema.union(["鹿", "鹿@用户", "补鹿", "戒鹿", "补鹿@用户", "戴锁"]).description("交互指令"),
        cost: import_koishi.Schema.number().description("货币变动")
      })).role("table").description("【获取硬币】本插件指令的货币变动").default(
  ...
  import_koishi.Schema.object({
    fontPath: import_koishi.Schema.string().description("渲染排行榜使用的字体（包含emoji）。<br>请填写`.ttf 字体文件`的绝对路径<br>若渲染功能正常，请不要修改此项！以免出现问题<br>仅供部分显示有问题的用户使用-> [Noto+Color+Emoji](https://fonts.google.com/noto/specimen/Noto+Color+Emoji)"),
    calendarImagePreset1: import_koishi.Schema.union([
      import_koishi.Schema.const("0").description("自定义路径（参见下方的路径选择配置项）"),
      import_koishi.Schema.const("1").description("鹿管（默认-预设1）"),
      import_koishi.Schema.const("2").description("心奈（预设2）")
    ]).role("radio").description("日历图片预设1-背景图").default("1"),
    calendarImagePreset2: import_koishi.Schema.union([
      import_koishi.Schema.const("0").description("自定义路径（参见下方的路径选择配置项）"),
      import_koishi.Schema.const("1").description("红勾（默认-预设1）"),
      import_koishi.Schema.const("2").description("壹佰分盖章（预设2）")
    ]).role("radio").description("日历图片预设2-完成符号").default("1")
  }).description("调试设置"),
  import_koishi.Schema.union([
    import_koishi.Schema.object({
      calendarImagePreset1: import_koishi.Schema.const("0").required(),
  ...
  import_koishi.Schema.union([
    import_koishi.Schema.object({
      calendarImagePreset2: import_koishi.Schema.const("0").required(),
  ...
  import_koishi.Schema.object({
    console: import_koishi.Schema.boolean().description("启用调试日志输出模式").default(false).experimental()
  }).description("开发者选项")
]);
...
async function apply(ctx, config) {
  ...
  async function readFileAsBase64(filePath) {
    const data = await import_node_fs.default.promises.readFile(filePath);
  ...
  try {
    ...
    if (fontPath) {
      const fontData = await import_node_fs.default.promises.readFile(fontPath);
  ...
  ctx.command("鹿管签到/鹿管购买 [item]", "购买签到道具", { authority: 1 }).example("鹿管购买 锁").example("鹿管购买 钥匙").userFields(["id", "name", "permissions"]).action(async ({ session }, item) => {
    const userId = session.userId;
    const costTable = getCostTable(config, session);
    const storeItems = costTable.store_item;
...
${availableItems}`);
  ...
  ctx.command("鹿管签到/看鹿 [user]", "查看用户签到日历", { authority: 1 }).alias("看🦌").alias("看看日历").userFields(["id", "name", "permissions"]).example("看鹿 @用户").action(async ({ session }, user) => {
    ...
    if (user) {
      ...
      if (parsedUser?.type === "at") {
        ...
        targetUserId = id;
        targetUsername = name2 || (typeof session.bot.getUser === "function" ? (await session.bot.getUser(targetUserId))?.name || targetUserId : targetUserId);
      } else {
  ...
  ctx.command("鹿管签到/鹿 [user]", "鹿管签到", { authority: 1 }).alias("🦌").userFields(["id", "name", "permissions"]).example("鹿").action(async ({ session }, user) => {
    if (user) {
    ...
    if (!sessionRecord) {
      ...
      if (currentSignInCount >= config.maximum_times_per_day) {
        const imgBuf2 = await renderSignInCalendar(ctx, sessionUserId, sessionUsername, currentYear, currentMonth);
    ...
    await updateUserCurrency(ctx, session.user.id, cost);
    if (config.enable_blue_tip) {
      await session.send(calendarImage + `<p>` + import_koishi.h.at(sessionUserId) + session.text(".Sign_in_success", [sessionRecord.totaltimes, cost]) + session.text(".enable_blue_tip"));
    } else {
      await session.send(calendarImage + `<p>` + import_koishi.h.at(sessionUserId) + session.text(".Sign_in_success", [sessionRecord.totaltimes, cost]));
    }
    return;
  });
  ctx.command("鹿管签到/帮鹿 [user]", "帮助用户签到", { authority: 1 }).alias("帮🦌").userFields(["id", "name", "permissions"]).example("帮鹿 @用户").action(async ({ session }, user) => {
    if (!user) {
      await session.execute("鹿");
      return;
    }
    ...
    let targetUsername = parsedUser.attrs.name || (typeof session.bot.getUser === "function" ? (await session.bot.getUser(targetUserId))?.name || targetUserId : targetUserId);
    const costTable1 = getCostTable(config, session, targetUserId);
    const costTable2 = getCostTable(config, session);
    const cost1 = costTable1.checkin_reward.find((c) => c.command === "鹿").cost;
    const cost2 = costTable2.checkin_reward.find((c) => c.command === "鹿@用户").cost;
    ...
    if (targetRecord && config.Reset_Cycle === "每月") {
      ...
      if (currentYear > recordYear || currentYear === recordYear && currentMonth > recordMonth) {
        await ctx.database.set("deerpipe", { userid: targetUserId }, {
          ...
          helpsignintimes: "",
          totaltimes: 0,
          allowHelp: targetRecord.allowHelp,
          // 保持原有的 allowHelp 设置
          itemInventory: targetRecord.itemInventory
          // 保持原有的道具
        });
    ...
    const targetUserIduid = await updateIDbyuserId(targetUserId, session);
    if (targetUserIduid === null) return;
    await updateUserCurrency(ctx, targetUserIduid, cost1);
    await updateUserCurrency(ctx, session.user.id, cost2);
    const imgBuf = await renderSignInCalendar(ctx, targetUserId, targetUsername, currentYear, currentMonth);
    const calendarImage = import_koishi.h.image(imgBuf, "image/png");
    const message = `${calendarImage}
${session.text(".Sign_in_success", [targetRecord.totaltimes, cost1])} `;
    await session.send(`${import_koishi.h.at(session.userId)} ${session.text(".Help_sign_in", [targetUsername, cost2])}`);
    if (config.enable_blue_tip) {
  ...
  ctx.command("鹿管签到/鹿榜", "查看签到排行榜", { authority: 1 }).alias("🦌榜").action(async ({ session }) => {
...
`;
  ...
  function getCostTable(config2, session, at_userId) {
    const specialCost = config2.special_cost;
    if (!at_userId) {
  ...
  __name(getCostTable, "getCostTable");
}
__name(apply, "apply");
