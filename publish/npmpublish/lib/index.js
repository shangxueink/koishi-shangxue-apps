...
`;
  ...
  async function getFriendStatusImg(ctx2, userData, botid, channelid, channelname) {
    ...
    await page.evaluate(
      (GroupHeadshotBase642, botname2, gamingUsers2, gamingUsersBase642, onlineUsers2, onlineUsersBase642, offlineUsers2, offlineUsersBase642, steamstatus2, allUserData2, showuserIdorsteamId) => {
        const findUserId = /* @__PURE__ */ __name((steamId) => {
          const user = allUserData2.find((u) => u.steamId === steamId);
        ...
        titles[0].innerHTML = `游戏中(${gamingUsers2.length})`;
        titles[1].innerHTML = `在线好友(${onlineUsers2.length})`;
        titles[2].innerHTML = `离线好友(${offlineUsers2.length})`;
        for (let i = 0; i < gamingUsers2.length; i++) {
          const li = document.createElement("li");
          li.setAttribute("class", "friend");
          li.innerHTML = `
                  <img src="${gamingUsersBase642[i]}" class="headshot-online">
                  <div class="name-and-status">
                      <p class="name-gaming">${gamingUsers2[i].personaname}(${showuserIdorsteamId ? gamingUsers2[i].steamid : findUserId(gamingUsers2[i].steamid)})</p>
                      <p class="status-gaming">${gamingUsers2[i].gameextrainfo}</p>
                  </div>`;
          gamingList.appendChild(li);
        }
        for (let i = 0; i < onlineUsers2.length; i++) {
          const li = document.createElement("li");
          li.setAttribute("class", "friend");
          li.innerHTML = `
                  <img src="${onlineUsersBase642[i]}" class="headshot-online">
                  <div class="name-and-status">
                      <p class="name-online">${onlineUsers2[i].personaname}(${showuserIdorsteamId ? onlineUsers2[i].steamid : findUserId(onlineUsers2[i].steamid)})</p>
                      <p class="status-online">${steamstatus2[onlineUsers2[i].personastate]}</p>
                  </div>`;
          onlineList.appendChild(li);
        }
        for (let i = 0; i < offlineUsers2.length; i++) {
          const li = document.createElement("li");
          li.setAttribute("class", "friend");
          li.innerHTML = `
                  <img src="${offlineUsersBase642[i]}" class="headshot-offline">
                  <div class="name-and-status">
                      <p class="name-offline">${offlineUsers2[i].personaname}(${showuserIdorsteamId ? offlineUsers2[i].steamid : findUserId(offlineUsers2[i].steamid)})</p>
                      <p class="status-offline">${steamstatus2[offlineUsers2[i].personastate]}</p>
                  </div>`;
