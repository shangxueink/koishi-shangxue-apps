...
function apply(ctx, config) {
  ...
  ctx.command(`${config.gifCommand} [gif:image]`).option("rebound", "-b, --rebound", { type: "boolean" }).option("reverse", "-r, --reverse", { type: "boolean" }).option("frame", "-f <frame:number>", { type: "number" }).option("slide", "-l <direction:string>", { type: "string" }).option("rotate", "-o <direction:string>", { type: "string" }).option("turn", "-t <angle:string>", { type: "string" }).option("information", "-i, --information", { type: "boolean" }).example(`回弹：${config.gifCommand} -b`).example(`倒放：${config.gifCommand} -r`).example(`指定帧间隔：${config.gifCommand} -f 20`).example(`右滑：${config.gifCommand} -l 右`).example(`逆时针旋转：${config.gifCommand} -o 逆`).example(`转向30度：${config.gifCommand} -t 30`).example(`转向向左上：${config.gifCommand} -t 左上`).example(`45度右滑：${config.gifCommand} -l 右 -t 45`).example(`顺时针旋转：${config.gifCommand} -o 顺`).example(`显示 GIF 信息: ${config.gifCommand} -i`).action(async ({ session, options }, gif) => {
