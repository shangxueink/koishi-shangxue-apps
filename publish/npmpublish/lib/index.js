...
function apply(ctx, config) {
  ...
  ctx.command("steam-friend-status/绑定steam <steamid:string>").option("id", "-i <id:string> 指定userid").option("name", "-n <name:string> 指定usename").usage("绑定steam账号，参数可以是好友码也可以是ID").example("绑定steam 123456789").example("绑定steam 123456789 -i 114514 -n 上学大人").action(async ({ session, options }, steamid) => {
    if (steamid == void 0) {
