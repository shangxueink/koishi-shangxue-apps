...
var Config = import_koishi.Schema.intersect([
  ...
  import_koishi.Schema.object({
    usedReverse: import_koishi.Schema.boolean().default(true).description("开启后，在不指定选项时，默认使用`倒放`效果。<br>关闭后，在不指定选项时，执行`-h`选项查看帮助")
  }).description("进阶设置"),
  import_koishi.Schema.object({
...
function apply(ctx, config) {
  ...
  ctx.command(`${config.gifCommand} [gif:image]`).option("rebound", "-b, --rebound", { type: "boolean" }).option("reverse", "-r, --reverse", { type: "boolean" }).option("speed", "-s <times:number>", { type: "number", fallback: 1 }).option("slide", "-l <direction:string>", { type: "string" }).option("rotate", "-o <direction:string>", { type: "string" }).option("mirror", "-m <direction:string>", { type: "string" }).example(`倒放：${config.gifCommand} -r`).example(`两倍速右滑：${config.gifCommand} -s 2 -l 右`).example(`向左翻转：${config.gifCommand} -m 左`).example(`逆时针旋转：${config.gifCommand} -o 逆`).action(async ({ session, options }, gif) => {
    let { reverse, rebound, speed, slide, rotate, mirror } = options;
    logInfo(options);
    if (!ctx.ffmpeg) return session.text(".invalidFFmpeg");
    if (Object.keys(options).length === 1 && options.speed === 1) {
    ...
    if (rebound) {
      totalDuration = gifDuration * 2 / speed;
      filters.push(
    ...
    builder.outputOption("-r", String(fps), "-loop", "0");
    if (vf) {
      logInfo(`使用的滤镜: ${vf}`);
