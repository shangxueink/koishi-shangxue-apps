...
${availableItems}`);
  ...
  ctx.command("鹿 [user]", "鹿管签到", { authority: 1 }).alias("🦌").userFields(["id", "name", "permissions"]).example("鹿").action(async ({ session }, user) => {
    if (user) {
  ...
  ctx.command("帮鹿 [user]", "帮助用户签到", { authority: 1 }).alias("帮🦌").userFields(["id", "name", "permissions"]).example("帮鹿 @用户").action(async ({ session }, user) => {
    if (!user) {
      await session.execute("鹿");
      return;
    }
