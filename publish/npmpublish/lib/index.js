    ...
    .action(async ({ session }, days) => {
      ...
      try {
        ...
        if (dataResponse.retcode === 0 && dataResponse.data && dataResponse.data.msg_data && dataResponse.data.msg_data.length > 0) {
          ...
          for (let i = 0; i < Math.min(displayDays, msgData.length); i++) { // 限制显示天数
            ...
            const content = session.text(`commands.${config.botdatacommandName}.messages.dataDisplay`, [`${botData['report_date']}`, dailyData]); // 修改 i18n 显示
