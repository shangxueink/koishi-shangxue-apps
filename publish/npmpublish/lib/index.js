...
exports.Config = Schema.intersect([
  ...
  Schema.object({
    autoEmoji: Schema.union(["取消应用", '定量消息', '定时发送']).description("打开后，开启自动表情包功能 <br>▶ 定量消息: `达到一定消息数量 自动触发表情包`<br>▶ 定时发送: `使用cron表达式定时触发表情包`此项需要`cron`服务").default("取消应用"),
  }).description('自动表情包设置'),
  Schema.union([
    Schema.object({
      autoEmoji: Schema.const("定量消息").required(),
      ...
      groupListmapping: Schema.array(Schema.object({
        ...
        count: Schema.number().default(30).description('触发自动表情包的消息数量的阈值'),
    ...
    Schema.object({
      ...
      groupListmapping: Schema.array(Schema.object({
        ...
        enable: Schema.boolean().description('勾选后 屏蔽该群 的自动表情包'),
      })).role('table').description('表情包指令映射 `注意群组ID不要多空格什么的`<br>私聊频道有`private:`前缀<br>▶ cron 定时语法 见 https://cron.koishi.chat/')
        .default([
          {
            "groupList": "114514",
...
function apply(ctx, config) {
  ...
  if (config.autoEmoji === "定量消息" && (config.groupListmapping.length || config.allgroupautoEmoji)) {
    const groups = {};
    // 初始化特定群组的配置
    config.groupListmapping.forEach(({ groupList, defaultemojicommand, count, enable }) => {
      // 只有当enable为false或未定义时，才将群组添加到启用列表中
      if (enable === false) {
        groups[groupList] = { count: 0, emojicommand: defaultemojicommand, threshold: count };
      } else {
      ...
      if (groupConfig) {
        ...
        if (groupConfig.count >= groupConfig.threshold) {
          ...
          if (randomNumber <= config.triggerprobability) {
            ...
            logInfo("定量消息：概率判断：此次不发送表情包，并且重置计数。")
          }
        }
      }
      return next();
  ...
  if (config.autoEmoji === "定时发送" && (config.groupListmapping.length || config.allgroupautoEmoji)) {
    const groups = {};
    // 初始化特定群组的配置
    ...
    for (const channelId in groups) {
      ...
      if (!groupConfig && config.allgroupautoEmoji) {
        // 初始化为全部群组的配置
        groups[channelId] = { emojicommand: config.allgroupemojicommand, cronTime: config.cronTime };
      }
              ...
              if (imageResult.imageUrl) {
                try {
                  ...
                  if (!channelId.includes("private")) {
                    ...
                    await bot.sendPrivateMessage(channelId, message);
                  }
