...
function apply(ctx, config) {
  ...
  ctx.command("steam-friend-status/绑定steam <steamid:string>").option("id", "-i <id:string> 指定userid").option("name", "-n <name:string> 指定usename").usage("绑定steam账号 steamid参数 可以是好友码 也可以是steamID").example("绑定steam 123456789").example("绑定steam 76561197960265728").example("绑定steam 123456789 -i 114514 -n 上学大人").example("绑定steam 76561197960265728 -i 114514 -n 上学大人").action(async ({ session, options }, steamid) => {
    if (steamid == void 0) {
  ...
  ctx.command("steam-friend-status/steam群报 <word:text>").usage("开启或关闭群通报").example("steam群报 on").example("steam群报 off").channelFields(["usingSteam"]).userFields(["authority"]).action(async ({ session }, text) => {
