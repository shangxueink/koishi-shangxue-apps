...
function apply(ctx, config) {
  ...
  ctx.command(`${config.gifCommand} [gif:image]`).option("forward", "-f, --forward  正向播放 GIF（默认）", { type: "boolean", fallback: true }).option("reverse", "-r, --reverse  倒放 GIF", { type: "boolean" }).option("speed", "-s <times:number>  改变播放速度 (大于 1 为加速，小于则为减速)", { fallback: 1 }).option("slide", "-l <direction:string>  滑动方向 (左/右/上/下)", { type: "string" }).option("rotate", "-o <direction:string>  旋转方向 (顺/逆)", { type: "string" }).option("mirror", "-m <direction:string>  翻转方向 (上/下/左/右)", { type: "string" }).example(`倒放：${config.gifCommand} -r`).example(`两倍速右滑：${config.gifCommand} -f -s 2 -l 右`).example(`向左翻转：${config.gifCommand} -m 左`).example(`逆时针旋转：${config.gifCommand} -o 逆`).action(async ({ session, options }, gif) => {
