...
export const name = 'music-link';
export const reusable = true; // 声明可重用
export const logger = new Logger('music-link');
export const inject = {
    required: ['http', "i18n"],
    optional: ['puppeteer'],
};
    ...
    ctx.on('ready', async () => {
                    ...
                    if (isSongId && !options.number) {
                        try {
                            ...
                            if (useApi === 'api.injahow.cn') {
                                songUrl = `https://api.injahow.cn/meting/?type=url&id=${isSongId}`;
                            } else if (useApi === 'meting.jmstrand.cn') {
                                songUrl = `https://meting.jmstrand.cn/?type=url&id=${isSongId}`;
                            } else if (useApi === 'api.qijieya.cn') {
                                songUrl = `https://api.qijieya.cn/meting/?type=url&id=${isSongId}`;
                            } else if (useApi === 'metingapi.nanorocky.top') {
                                songUrl = `https://metingapi.nanorocky.top/?server=netease&type=url&id=${isSongId}`;
                            }
            ...
            ctx.command(`${config.command8} <keyword:text>`)
                ...
                .action(async ({ session, options }, keyword) => {
                    ...
                    if (index) {
                        ...
                        if (!songChoice) {
                            return `${session.text(`.waitTimeout`)}`;
                        }
        ...
        async function generateResponse(session, data, platformconfig) {
            ...
            if (config.isfigure && (session.platform === "onebot" || session.platform === "red")) {
                ...
                (responseElements as any) = responseElements.join('\n')
