...
但还暂未支持源网站的字体渲染捏，因为字体真大（（（ 

推荐使用 [puppeteer-without-canvas](/market?keyword=puppeteer-without-canvas) 的puppeteer服务。

（可以设置全局默认字体，以提升本插件渲染结果的观感。）
...
export const Config: Schema<Config> = Schema.intersect([
  ...
  Schema.object({
    ...
    defaultPromptTime: Schema.number().default(60).description("用户问答输入的超时时间。（单位：秒）<br>超时自动取消生成，退出交互。"),
  ...
  ctx.on('ready', () => {
    ...
    async function renderCharacterCard(ctx: Context, characterData: CharacterData): Promise<Buffer | null> {
      try {
        ...
        if (!htmlContent) {
          ctx.logger.error('角色卡片生成失败: htmlContent: ', htmlContent);
          return null;
        }
