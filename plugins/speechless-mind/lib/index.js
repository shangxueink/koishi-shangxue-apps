...
exports.Config = Schema.intersect([
    Schema.object({
        ...
        Watermarkposition: Schema.tuple([Number, Number]).default([-10, 0]).description("用于调整水滴位置<br>分别代表【水滴】的top位置和right位置"),
        Watermarkpath: Schema.path().description("【水滴】图片素材的路径").default(path.join(__dirname, '../png/1.png')),
    }).description('基础设置'),


    Schema.object({
        //  AllowLineWrap: Schema.boolean().default(false).description("开启后，在 fastmakemode 关闭时，允许自定义文本换行。"),
    ...
    ctx.command("无语 [text] [img]", "无语思维插件")
        ...
        .action(async ({ session }, text, img) => {
            ...
            try {
                ...
                const idealAspectRatio = 1; // 理想比例为1:1
                ...
                if (config.fastmakemode) {
                    ...
                    //  textContent = config.AllowLineWrap ? text : text.replace(/\n/g, '<br>');
                }
                // 加载HTML模板并插入动态内容
