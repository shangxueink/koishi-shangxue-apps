...
export class WsClient<C extends Context = Context> extends Adapter.WsClient<C, QQBot<C>> {
  ...
  async prepare() {
    ...
    try {
      //let { url } = this.bot.config.authType
      //url = url.replace('api.sgroup.qq.com', new URL(this.bot.config.endpoint).host)
      this.bot.logger.debug(this.bot.config.webhookURL) // wss://api.sgroup.qq.com/websocket
      return this.bot.http.ws(this.bot.config.webhookURL) //wss://webhook.company/ws/PoEe4UuKkAa1StKlCd4WyQsKmEg8b4X0
    } catch (error) {
